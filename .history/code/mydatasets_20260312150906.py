...
def load_seizure_dataset(path, model_type):
	...
	X = df.drop(df.columns[-1], axis=1).to_numpy(dtype=np.float32)
	y = df[df.columns[-1]].to_numpy(dtype=np.int64) - 1
	...
	if model_type == 'MLP':
		...
		data = torch.tensor(X, dtype=torch.float32).unsqueeze(2) 
		target = torch.tensor(y, dtype=torch.long)
		dataset = TensorDataset(data, target)
	else:
...
class VisitSequenceWithLabelDataset(Dataset):
	def __init__(self, seqs, labels, num_features):
		...
		for patient_visits in seqs:
			...
			# Build COO data for a sparse matrix
			rows, cols, data = [], [], []
			for visit_idx, visit_codes in enumerate(patient_visits):
				for code in visit_codes:
					rows.append(visit_idx)
					cols.append(code)
					data.append(1.0)
			mat = sparse.csr_matrix(
				(data, (rows, cols)),
				shape=(num_visits, num_features),
				dtype=np.float32
			)
